...
module "ecs_cluster_api_xpto" {
  ...
  namespace_arn             = var.arn_namespace_api_xpto
...
#----------# ALB development #-----------#
module "alb_api_jpn_jpn" {
  source             = "git::https://github.com/JP-Neto/Terraform-Multi-Cloud-Modules.git//modules/aws/compute/loadbalancer?ref=main"

  lb_name            = var.api_jpn_jpn_name
...
module "alb_listener_jpn_443" {
  ...
  lb_arn           = module.alb_api_jpn_jpn.lb_arn
  ...
  target_group_arn = module.tg_api_jpn_jpn.target_group_arn
  
  tags = merge(local.common_tags,var.listener_api_jpn_tags)

  depends_on = [module.alb_api_jpn_jpn, module.tg_api_jpn_jpn]
}
...
module "alb_listener_jpn_80" {
  ...
  depends_on       = [module.alb_api_jpn_jpn]
}
